...
<div class="form-group row align-items-center"
    ...
    <label for="parent" class="col-form-label text-md-right"
        :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.category.columns.parent') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect v-model="form.parent" label="name" :options="{{ $categories->toJSON() }}" :option-height="104"
            placeholder="{{ trans('admin.category.columns.parent') }}" track-by="id">
        </multiselect>
        <div v-if="errors.has('parent')" class="form-control-feedback form-text" v-cloak>
            @{{ errors . first('parent') }}</div>
    </div>
</div>
